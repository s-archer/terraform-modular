...
resource "local_file" "rendered_cs_dns_lb" {
  ...
  filename = "${path.module}/output_files/f5cs4_rendered_cs_dns_lb_${count.index}.json"
}
...
resource "null_resource" "cs_create_dns_lb" {
  ...
  provisioner "local-exec" {
    command = "curl -k -X POST -H 'Content-type: application/json' -H 'Authorization: Bearer ${jsondecode(data.local_file.f5cs_login_response.content).access_token}' --data-binary '@${path.module}/output_files/f5cs4_rendered_cs_dns_lb_${count.index}.json' https://api.cloudservices.f5.com/v1/svc-subscription/subscriptions > ${path.module}/output_files/f5cs5_create_dns_lb_${count.index}.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/output_files/f5cs5_create_dns_lb_${count.index}.json"
  }
}
...
data "local_file" "f5cs5_create_dns_lb" {
  count      = var.app_count
  filename   = "${path.module}/output_files/f5cs5_create_dns_lb_${count.index}.json"
...
resource "null_resource" "cs_activate_dns_lb" {
  ...
  provisioner "local-exec" {
    command = "curl -k -X POST -H 'Content-type: application/json' -H 'Authorization: Bearer ${jsondecode(data.local_file.f5cs_login_response.content).access_token}' https://api.cloudservices.f5.com/v1/svc-subscription/subscriptions/${jsondecode(data.local_file.f5cs5_create_dns_lb[count.index].content).subscription_id}/activate > ${path.module}/output_files/f5cs6_activate_dns_lb_${count.index}.json"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "rm -f ${path.module}/output_files/f5cs6_activate_dns_lb_${count.index}.json"
  }
}
